...
export const ProviderNotifyc = () => {
  ...
  const filterModuls = useMemo(
    () => products.filter((product) => product.type === "provider"),
    [products]
  );
  const deleteproduc = async (id) => {
    ...
    if (response.status === 200) {
      messageSuccess("Notificación eliminada correctamente");
      const data = await getNotification();
      setProducts(data.data.responseNotification);
      setLoading2(false);
    } else {
      messageError("Error al eliminar la notificación");
    }
  };
